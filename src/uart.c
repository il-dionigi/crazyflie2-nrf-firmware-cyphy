...
static bool isInitOther = false; // Init for secondary UART connection
...
void uartInit()
{
  if (!isInit) {
    ...
    isInit = true;
  } else if (!isInitOther) { // Init for secondary UART
    NRF_GPIO1->PIN_CNF[8] &= ~GPIO_PIN_CNF_PULL_Msk;
    NRF_GPIO1->PIN_CNF[8] |= (GPIO_PIN_CNF_PULL_Pulldown<<GPIO_PIN_CNF_PULL_Pos);

    NRF_GPIO1->PIN_CNF[UART_TX_PIN1] = (NRF_GPIO1->PIN_CNF[UART_TX_PIN1] & (~GPIO_PIN_CNF_DRIVE_Msk)) | (GPIO_PIN_CNF_DRIVE_S0S1<<GPIO_PIN_CNF_DRIVE_Pos);

    NRF_GPIO1->DIRSET = 1<<UART_TX_PIN1;
    NRF_GPIO1->OUTSET = 1<<UART_TX_PIN1;
    NRF_UART1->PSELTXD = UART_TX_PIN1;

    NRF_GPIO1->DIRCLR = 1<<UART_RX_PIN1;
    NRF_UART1->PSELRXD = UART_RX_PIN1;

    NRF_GPIO1->DIRSET = 1<<UART_RTS_PIN;
    NRF_GPIO1->OUTSET = 1<<UART_RTS_PIN;
    NRF_UART1->PSELRTS = UART_RTS_PIN;

    NRF_UART1->CONFIG = UART_CONFIG_HWFC_Msk;

  #ifndef DEBUG_UART
    NRF_UART1->BAUDRATE = UART_BAUDRATE_BAUDRATE_Baud1M;
  #else
    NRF_UART1->BAUDRATE = UART_BAUDRATE_BAUDRATE_Baud460800;
  #endif

    NRF_UART1->ENABLE = UART_ENABLE_ENABLE_Enabled;

    NRF_UART1->TASKS_STARTTX = 1;

    // Enable interrupt on receive
    NVIC_SetPriority(UART0_IRQn, 3);
    NVIC_EnableIRQ(UART0_IRQn);
    NRF_UART1->INTENSET = UART_INTENSET_RXDRDY_Msk;

    NRF_UART1->TASKS_STARTRX = 1;
...
void uartDeinit()
{
  ...
  nrf_gpio_cfg_input(UART_TX_PIN2, NRF_GPIO_PIN_NOPULL);
