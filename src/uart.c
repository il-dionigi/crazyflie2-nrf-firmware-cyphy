...
void uartInit()
{
  if (!isInit) {
    ...
    NRF_GPIO1->PIN_CNF[UART_TX_PIN] = (NRF_GPIO1->PIN_CNF[UART_TX_PIN] & (~GPIO_PIN_CNF_DRIVE_Msk)) | (GPIO_PIN_CNF_DRIVE_S0S1<<GPIO_PIN_CNF_DRIVE_Pos);

    NRF_GPIO1->DIRSET = 1<<UART_TX_PIN;
    NRF_GPIO1->OUTSET = 1<<UART_TX_PIN;
    NRF_UART1->PSELTXD = UART_TX_PIN;

    NRF_GPIO1->DIRCLR = 1<<UART_RX_PIN;
    NRF_UART1->PSELRXD = UART_RX_PIN;
